...
function GalleryScene({
	...
	const planesData = useRef<PlaneData[]>(
		Array.from({ length: visibleCount }, (_, i) => ({
			index: i,
			// Distribute initially throughout the range
			z: (depthRange / visibleCount) * i, 
	...
	useEffect(() => {
		planesData.current = Array.from({ length: visibleCount }, (_, i) => ({
			index: i,
			z: (depthRange / Math.max(visibleCount, 1)) * i,
			imageIndex: totalImages > 0 ? i % totalImages : 0,
			x: spatialPositions[i]?.x ?? 0,
			y: spatialPositions[i]?.y ?? 0,
		}));
	...
	// Auto-play logic with limit
	useEffect(() => {
		const interval = setInterval(() => {
			// Limit auto-play to a certain duration
	...
	useFrame((state, delta) => {
		...
		planesData.current.forEach((plane, i) => {
			...
			// Stop at boundaries (finite scroll)
			if (newZ >= totalRange) {
				newZ = totalRange;
			} else if (newZ < 0) {
				newZ = 0;
